 ...
 if (dim0 > 0)
   {
	 ...
	 *ptriArray = malloc(dim0*dim1*sizeof(REAL));
       }
     if (triSizeSet == 1)
       *triSize1 = dim1;
				       ...
				       int forceResize, int triSizeSet)
{
 ...
 if (dim0 > 0)
   {
     if (*triSize0 != dim0 || *triSize1 != dim1 || forceResize > 0)
       {
	 if (*ptriArray) free(*ptriArray);
	 *ptriArray = malloc(dim0*dim1*sizeof(int));
       }
     if (triSizeSet == 1)
       *triSize1 = dim1;
     else if (triSizeSet == 0)
 ...
 araw = (int *) array->data;
#ifdef DEBUG_UTIL
 printf("copyToTriIntArray dim0= %d dim1= %d triSizeSet= %d ptriArray= %x\n",
	dim0,dim1,triSizeSet,*ptriArray);
#endif
...
{
  PyObject *address0,*address1,*address2, *flagsin,*rval;
  ...
  in  = PyCObject_AsVoidPtr(address0);  
  out = PyCObject_AsVoidPtr(address1);  
  vor = PyCObject_AsVoidPtr(address2);
...
{
  PyObject *address0,*address1, *flagsin,*rval;
  struct triangulateio *in,*out,*vor;
  char *flags;
 ...
 **************************************************/
static char triangulate_doc[] = "provide interface to triangulateio type";
...
/***************************************************
 method table
 **************************************************/
static PyMethodDef triangulateMethods[] = {
...
PyMODINIT_FUNC inittriangulate()
{
  PyObject *m,*d;
  m = Py_InitModule3("triangulate", triangulateMethods,triangulate_doc);
