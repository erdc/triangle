...
def tricall3():
    ...
    mesh1.viewShowme()
#end triIfaceCall3
def exAdhPyUtilLaplace1(filebase="trimesh",baseFlags="zen",
    ...
    if useCG:
        if order == 1:
            FemSpace = FemTools.C0_AffineLinearOnSimplexWithNodalBasis(adhPyMesh,nd)
        else:
            FemSpace = FemTools.C0_AffineQuadraticOnSimplexWithNodalBasis(adhPyMesh,nd)
    else:
        if order == 1:
            FemSpace = FemTools.DG_AffineLinearOnSimplexWithNodalBasis(adhPyMesh,nd)
        else:
            FemSpace = FemTools.DG_AffineQuadraticOnSimplexWithNodalBasis(adhPyMesh,nd)
...
if __name__ == '__main__':
    ...
    exAdhPyUtilLaplace1(filebase="../examples/la",baseFlags="en",
                        flagsAdd=flagsAdd,viewMesh=0,verbose=0)
